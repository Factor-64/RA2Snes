...
void MemoryReader::processFrames(const char* data, unsigned int& frames)
{
    //QElapsedTimer timer;
    //timer.start();

    memory_t mem;
    mem.ram = reinterpret_cast<uint8_t*>(const_cast<char*>(data));
    mem.size = sizeof(mem.ram);
    ...
    //qDebug() << "Execution time (ns):" << elapsed;
}
